$(function() {
    ...
    function validateY() {
        ...
        if (isNumeric(numY) && numY > Y_LOW_BOUND && numY < Y_TOP_BOUND)
        {
            $('.input-form__info').text('Введите координаты точки')
            return true;
        } else {
            $('.input-form__info').text('Введите значение Y от -5 до 5 невключительно')
            return false;
        }
    }
    ...
    function validateR() {
        ...
        // let numR = $('.input-form__text_r').val().replace(',', '.');
        //
        // if (isNumeric(numY) && numY > Y_LOW_BOUND && numY < Y_TOP_BOUND)
        // {
        //     $('.input-form__info').text('Введите координаты точки')
        ...
        if (rval !== undefined) {
            ...
            $('.input-form__info').text('Введите значение R от 1 до 4 невключительно')
            return false;
        }
    }
    ...
    canvas.on('click', function (event) {
        ...
        if (yValue <= Y_LOW_BOUND) yValue = Y_LOW_BOUND + 0.000001;
        else if (yValue >= Y_TOP_BOUND) yValue = Y_TOP_BOUND - 0.000001;
